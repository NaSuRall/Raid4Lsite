  ...
  for (const element of elements || []) {
    ...
    try {
      if (element.type === 'text') {
        ...
        const titleHeight = title ? doc.heightOfString(title, { width: maxWidth, align: 'center' }) : 0;
        doc.font(font.regular).fontSize(descriptionSize);
        const descriptionHeight = description
          ? doc.heightOfString(description, { width: maxWidth, align: 'center', lineGap: 2 })
          : 0;
        ...
        if (title) {
          doc.font(font.bold).fontSize(fontSize).text(title, x - maxWidth / 2, cursorY, {
            width: maxWidth, align: 'center', lineGap: 1,
          });
          cursorY += titleHeight + textGap;
        }
        if (description) {
          doc.font(font.regular).fontSize(descriptionSize).text(description, x - maxWidth / 2, cursorY, {
            width: maxWidth, align: 'center', lineGap: 2,
          });
